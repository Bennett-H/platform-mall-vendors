<template>
  <div class="mod-order">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList(1)">
      ...
      <el-form-item>
        <el-select v-model="searchForm.fromType" clearable placeholder="下单来源" @change="getDataList(1)">
          <el-option
            key="1"
            label="微信小程序"
            value="1">
      ...
      <el-form-item>
        <el-select v-model="searchForm.orderType" clearable placeholder="订单类型" @change="getDataList(1)">
          <el-option
            key="1"
            label="商城订单"
            value="1">
      ...
      <el-form-item>
        <el-select v-model="searchForm.payStatus" clearable placeholder="付款状态" @change="getDataList(1)">
          <el-option
            key="1"
            label="未付款"
            value="1">
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.fromType === 7" size="small" type="warning">头条小程序</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        width="120px"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.orderStatus === 400" size="small" effect="dark" type="danger">申请退款</el-tag>
...
export default {
  ...
  methods: {
    // 导出excel
    exportHandle () {
      let url = this.$http.BASE_URL + `/mall/order/exportMyOrderExcel?nickname=${this.searchForm.nickname}&orderSn=${this.searchForm.orderSn}&fromType=${this.searchForm.fromType}&orderType=${this.searchForm.orderType}&payStatus=${this.searchForm.payStatus}&shippingStatus=${this.searchForm.shippingStatus}&consignee=${this.searchForm.consignee}&mobile=${this.searchForm.mobile}&token=${this.$cookie.get('token')}`
      window.open(url)
    },
    // 查看会员详情
    showUserDetails (id) {
