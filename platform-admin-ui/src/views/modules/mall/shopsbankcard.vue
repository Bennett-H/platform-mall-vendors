<template>
  <div class="mod-userbankcard">
    <el-form :inline="true" :model="searchForm" @keyup.enter.native="getDataList(1)">
      ...
      <el-form-item>
        <el-select v-model="searchForm.cardStatus" clearable placeholder="状态" @change="getDataList(1)">
          <el-option
            key="2"
            label="已解绑"
            value="2">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('mall:shopsbankcard:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('mall:shopsbankcard:delete')" type="danger" @click="deleteHandle()"
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button v-if="isAuth('mall:userbankcard:update')" type="text" size="small"
                     @click="showDetails(scope.row.id)">修改
          </el-button>
          <el-button v-if="isAuth('mall:userbankcard:delete')" type="text" size="small"
...
<script>
import AddOrUpdate from './shopsbankcard-add-or-update.vue'
