<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <div class="padding text-gray">测试号可选择服务号，不同类型账号、是否认证可使用功能权限不同，<a
        href="https://developers.weixin.qq.com/doc/offiaccount/Getting_Started/Explanation_of_interface_privileges.html">参考文档</a>
      </div>
      <el-row>
        <el-col :span="12">
      ...
      <el-form-item label="appid" prop="appid">
        <el-input v-model="dataForm.appid" :disabled="disabled" placeholder="appid"></el-input>
      </el-form-item>
      ...
      <el-form-item label="content" prop="content">
        <el-input type="textarea" v-model="dataForm.content" placeholder="关注后自动回复"></el-input>
      </el-form-item>
    </el-form>
  ...
  methods: {
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: `/manage/wxAccount/${!this.dataForm.appid ? 'save' : 'update'}`,
            method: 'post',
            data: this.dataForm
          }).then(({data}) => {
            if (data && data.code === 0) {
