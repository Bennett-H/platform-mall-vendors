...
!(function () {
  !(function () {
    UE = window.UE || {}; var a = !!window.ActiveXObject, b = {removeLastbs: function (a) {
        ...
        extend: function (a, b) {
          for (var c = arguments, d = !!this.isBoolean(c[c.length - 1]) && c[c.length - 1], e = this.isBoolean(c[c.length - 1]) ? c.length - 1 : c.length, f = 1; f < e; f++) {
            var g = c[f]; for (var h in g) {
              d && a.hasOwnProperty(h) || (a[h] = g[h])
            }
          } return a
        },
        isIE: a,
        cssRule: a ? function (a, b, c) {
          var d, e; c = c || document, d = c.indexList ? c.indexList : c.indexList = {}; var f; if (d[a]) {
            f = c.styleSheets[d[a]]
          } else {
            if (void 0 === b) {
              return ''
          ...
          c = c || document; var d, e = c.getElementsByTagName('head')[0]; if (!(d = c.getElementById(a))) {
            if (void 0 === b) {
              return ''
        ...
        domReady: function (b) {
          var c = window.document; c.readyState === 'complete' ? b() : a ? (!(function () {
            if (!c.isReady) {
              try {
    ...
    b()
        ...
        inArray: function (a, b) {
          var c = -1; return this.each(a, function (a, d) {
            if (a === b) {
              return c = d, !1
            }
          }), c
        ...
        hasClass: function (a, b) {
          b = b.replace(/(^[ ]+)|([ ]+$)/g, '').replace(/[ ]{2,}/g, ' ').split(' '); for (var c, d = 0, e = a.className; c = b[d++];) {
            if (!new RegExp('\\b' + c + '\\b', 'i').test(e)) {
              return !1
            }
          } return d - 1 == b.length
        },
        addClass: function (a, c) {
          if (a) {
            c = this.trim(c).replace(/[ ]{2,}/g, ' ').split(' '); for (var d, e = 0, f = a.className; d = c[e++];) {
              new RegExp('\\b' + d + '\\b').test(f) || (f += ' ' + d)
            } a.className = b.trim(f)
          }
        },
        removeClass: function (a, b) {
          b = this.isArray(b) ? b : this.trim(b).replace(/[ ]{2,}/g, ' ').split(' '); for (var c, d = 0, e = a.className; c = b[d++];) {
            e = e.replace(new RegExp('\\b' + c + '\\b'), '')
          }e = this.trim(e).replace(/[ ]{2,}/g, ' '), a.className = e, !e && a.removeAttribute('className')
        },
        on: function (a, c, d) {
          var e = this.isArray(c) ? c : c.split(/\s+/), f = e.length; if (f) {
            for (;f--;) {
              if (c = e[f], a.addEventListener) {
                a.addEventListener(c, d, !1)
              } else {
                d._d || (d._d = {els: []}); var g = c + d.toString(), h = b.indexOf(d._d.els, a); d._d[g] && h != -1 || (h == -1 && d._d.els.push(a), d._d[g] || (d._d[g] = function (a) {
          ...
          var e = this.isArray(c) ? c : c.split(/\s+/), f = e.length; if (f) {
            for (;f--;) {
              if (c = e[f], a.removeEventListener) {
                ...
                var g = c + d.toString(); try {
          a.detachEvent('on' + c, d._d ? d._d[g] : d)
        } catch (h) {} if (d._d && d._d[g]) {
     var i = b.indexOf(d._d.els, a); i != -1 && d._d.els.splice(i, 1), d._d.els.length == 0 && delete d._d[g]
   }
              }
            }
          }
        ...
        loadFile: (function () {
          ...
          return d
            ...
            var f = a(c, d); if (f) {
              return void (f.ready ? e && e() : f.funs.push(e))
  ...
  var i = c.createElement(d.tag); delete d.tag; for (var h in d) {
     ...
     throw Error('The load ' + (d.href || d.src) + ' fails,check the url')
        ...
        }); var c = {}; UE.parse = {register: function (a, b) {
          c[a] = b
        },
          load: function (a) {
            b.each(c, function (c) {
    ...
    b.domReady(function () {
      var d; if (document.querySelectorAll) {
    ...
    var b = this.root.getElementsByTagName('pre'); if (b.length) {
    ...
    function b (b, c) {
      var d, e = b; for (c = a.isArray(c) ? c : [c]; e;) {
        for (d = 0; d < c.length; d++) {
          if (e.tagName == c[d].toUpperCase()) {
          return e
        }
        } e = e.parentNode
      } return null
    } function c (b, c, e) {
      for (var f = b.rows, g = [], h = f[0].cells[0].tagName === 'TH', i = 0, j = 0, k = f.length; j < k; j++) {
        g[j] = f[j]
      } var l = {reversecurrent: function (a, b) {
      return 1
    },
      orderbyasc: function (a, b) {
         var c = a.innerText || a.textContent, d = b.innerText || b.textContent; return c.localeCompare(d)
       },
      reversebyasc: function (a, b) {
         var c = a.innerHTML, d = b.innerHTML; return d.localeCompare(c)
       },
      orderbynum: function (b, c) {
         var d = b[a.isIE ? 'innerText' : 'textContent'].match(/\d+/), e = c[a.isIE ? 'innerText' : 'textContent'].match(/\d+/); return d && (d = +d[0]), e && (e = +e[0]), (d || 0) - (e || 0)
       },
      reversebynum: function (b, c) {
         var d = b[a.isIE ? 'innerText' : 'textContent'].match(/\d+/), e = c[a.isIE ? 'innerText' : 'textContent'].match(/\d+/); return d && (d = +d[0]), e && (e = +e[0]), (e || 0) - (d || 0)
       }}; b.setAttribute('data-sort-type', e && typeof e === 'string' && l[e] ? e : ''), h && g.splice(0, 1), g = d(g, function (a, b) {
        var d; return d = e && typeof e === 'function' ? e.call(this, a.cells[c], b.cells[c]) : e && typeof e === 'number' ? 1 : e && typeof e === 'string' && l[e] ? l[e].call(this, a.cells[c], b.cells[c]) : l.orderbyasc.call(this, a.cells[c], b.cells[c])
      }); for (var m = b.ownerDocument.createDocumentFragment(), n = 0, k = g.length; n < k; n++) {
       m.appendChild(g[n])
     } var o = b.getElementsByTagName('tbody')[0]; i ? o.insertBefore(m, f[i - range.endRowIndex + range.beginRowIndex - 1]) : o.appendChild(m)
    } function d (a, b) {
      b = b || function (a, b) {
    ...
    if (!a.hasClass(b.rows[0], 'firstRow')) {
     ...
     var i = this.selector; a.cssRule('table', i + ' table.noBorderTable td,' + i + ' table.noBorderTable th,' + i + ' table.noBorderTable caption{border:1px dashed #ddd !important}' + i + ' table.sortEnabled tr.firstRow th,' + i + ' table.sortEnabled tr.firstRow td{padding-right:20px; background-repeat: no-repeat;background-position: center right; background-image:url(' + this.rootPath + 'themes/default/images/sortable.png);}' + i + ' table.sortEnabled tr.firstRow th:hover,' + i + ' table.sortEnabled tr.firstRow td:hover{background-color: #EEE;}' + i + ' table{margin-bottom:10px;border-collapse:collapse;display:table;}' + i + ' td,' + i + ' th{ background:white; padding: 5px 10px;border: 1px solid #DDD;}' + i + ' caption{border:1px dashed #DDD;border-bottom:0;padding:3px;text-align:center;}' + i + ' th{border-top:1px solid #BBB;background:#F7F7F7;}' + i + ' table tr.firstRow th{border-top:2px solid #BBB;background:#F7F7F7;}' + i + ' tr.ue-table-interlace-color-single td{ background: #fcfcfc; }' + i + ' tr.ue-table-interlace-color-double td{ background: #f7faff; }' + i + ' td p{margin:0;padding:0;}', document), a.each('td th caption'.split(' '), function (b) {
      var c = g.getElementsByTagName(b); c.length && a.each(c, function (a) {
     ...
     /\bsortEnabled\b/.test(d.className) && a.on(d, 'click', function (d) {
      var g = d.target || d.srcElement, h = b(g, ['td', 'th']), i = b(g, 'table'), j = a.indexOf(i.rows[0].cells, h), k = i.getAttribute('data-sort-type'); j != -1 && (c(i, j, f.tableSortCompareFn || k), e(i))
    })
   })
   }
    ...
    function b () {
      ...
      b.getAttribute('data-chart') !== null && c.push(d(b))
    } return c.length ? c : null
    } function d (a) {
    for (var b, c = a.getAttribute('data-chart'), d = {}, e = [], f = 0; b = a.rows[f]; f++) {
       for (var g, h = [], i = 0; g = b.cells[i]; i++) {
        var j = g.innerText || g.textContent || ''; h.push(g.tagName == 'TH' ? j : 0 | j)
      }e.push(h)
     }c = c.split(';'); for (var k, f = 0; k = c[f]; f++) {
      k = k.split(':'), d[k[0]] = k[1]
  ...
  var b = document.createElement('div'); return b.className = 'edui-chart-container', a.parentNode.replaceChild(b, a), b
} function l (a) {
  var b = [], c = [], d = [], e = a.data, f = a.meta; if (f.dataFormat != '1') {
    ...
    c = e[0].slice(1); for (var k, g = 1; k = e[g]; g++) {
    ...
    function b (b) {
    var e = this; a.each(b, function (b) {
       if (b.className && /custom_/i.test(b.className)) {
        var f = b.className.match(/custom_(\w+)/)[1]; if (f == 'dash' || f == 'dot') {
          a.pushItem(c, h + ' li.list-' + d[f] + '{background-image:url(' + e.liiconpath + d[f] + '.gif)}'), a.pushItem(c, h + ' ul.custom_' + f + '{list-style:none;} ' + h + ' ul.custom_' + f + ' li{background-position:0 3px;background-repeat:no-repeat}')
        } else {
          var g = 1; a.each(b.childNodes, function (b) {
            b.tagName == 'LI' && (a.pushItem(c, h + ' li.list-' + d[f] + g + '{background-image:url(' + e.liiconpath + 'list-' + d[f] + g + '.gif)}'), g++)
  ...
  var b = this.root.getElementsByTagName('video'), c = this.root.getElementsByTagName('audio'); if (document.createElement('video'), document.createElement('audio'), b.length || c.length) {
     var d = a.removeLastbs(this.rootPath), e = d + '/third-party/video-js/video.js', f = d + '/third-party/video-js/video-js.min.css', g = d + '/third-party/video-js/video-js.swf'; window.videojs ? videojs.autoSetup() : (a.loadFile(document, {id: 'video_css', tag: 'link', rel: 'stylesheet', type: 'text/css', href: f}), a.loadFile(document, {id: 'video_js', src: e, tag: 'script', type: 'text/javascript'}, function () {
