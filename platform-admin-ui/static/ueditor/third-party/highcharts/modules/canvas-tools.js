...
function RGBColor (m) {
  this.ok = !1; m.charAt(0) == '#' && (m = m.substr(1, 6)); var m = m.replace(/ /g, ''), m = m.toLowerCase(), a = {aliceblue: 'f0f8ff',
      ...
      whitesmoke: 'f5f5f5',
      yellow: 'ffff00',
      yellowgreen: '9acd32'}, c; for (c in a) {
        m == c && (m = a[c])
      } var d = [{re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/,
        ...
        return [parseInt(b[1] + b[1], 16), parseInt(b[2] + b[2], 16), parseInt(b[3] + b[3], 16)]
         ...
         var b = d[c].process, k = d[c].re.exec(m); if (k) {
...
: this.g > 255 ? 255 : this.g; this.b = this.b < 0 || isNaN(this.b) ? 0 : this.b > 255 ? 255 : this.b; this.toRGB = function () {
  ...
  var b = this.r.toString(16), a = this.g.toString(16), d = this.b.toString(16); b.length == 1 && (b = '0' + b); a.length == 1 && (a = '0' + a); d.length == 1 && (d = '0' + d); return '#' + b + a + d
...
(b /= 100); return b
  ...
  }; var b = this; this.Color = {addOpacity: function (d) {
    var c = b.value; if (d != null && d != '') {
      var f = new RGBColor(b.value); f.ok && (c = 'rgba(' + f.r + ', ' + f.g + ', ' + f.b + ', ' + d + ')')
    } return new a.Property(b.name, c)
  }}; this.Definition = {getDefinition: function () {
    var d = b.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2'); return a.Definitions[d]
  },
    isUrl: function () {
...
0
    ...
    getFillStyle: function (b) {
      var d = this.getDefinition(); return d != null && d.createGradient ? d.createGradient(a.ctx, b) : d != null && d.createPattern ? d.createPattern(a.ctx, b) : null
    }}; this.Length = {DPI: function () {
     return 96
   },
     EM: function (b) {
      var d = 12, c = new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize); c.hasValue() && (d = c.Length.toPixels(b)); return d
    },
     toPixels: function (d) {
      if (!b.hasValue()) {
        return 0
...
? b.numValue() : c.match(/pt$/) ? b.numValue() * 1.25 : c.match(/pc$/) ? b.numValue() * 15 : c.match(/cm$/) ? b.numValue() * this.DPI(d) / 2.54 : c.match(/mm$/) ? b.numValue() * this.DPI(d) / 25.4 : c.match(/in$/) ? b.numValue() * this.DPI(d) : c.match(/%$/) ? b.numValue() * a.ViewPort.ComputeSize(d) : b.numValue()
      ...
      if (!b.hasValue()) {
        return 0
      } var a = b.value + ''; if (a.match(/s$/)) {
        return b.numValue() * 1E3
      } a.match(/ms$/); return b.numValue()
...
function (b) {
  var a = this.x * b[1] + this.y * b[3] + b[5]; this.x = this.x * b[0] + this.y * b[2] + b[4]; this.y = a
}
...
a); this.addBezierCurve(b, a, d, d + 1 / 3 * (k - b), c, c + 1 / 3 * (l - a), k, l)
  ...
  var q = [b, a], p = [d, c], t = [k, l], m = [o, n]; this.addPoint(q[0], q[1]); this.addPoint(m[0], m[1]); for (i = 0; i <= 1; i++) {
    ...
    }, a = 6 * q[i] - 12 * p[i] + 6 * t[i], d = -3 * q[i] + 9 * p[i] - 9 * t[i] + 3 * m[i], c = 3 * p[i] - 3 * q[i], d == 0 ? a != 0 && (a = -c / a, a > 0 && a < 1 && (i == 0 && this.addX(b(a)), i == 1 && this.addY(b(a)))) : (c = Math.pow(a, 2) - 4 * c * d, c < 0 || (k =
...
new a.Property('angle', b)
  ...
  this.applyToPoint = function (b) {
    ...
    var k = c[b].split('(')[0], e = c[b].split('(')[1].replace(')', ''); this.transforms.push(new this.Type[k](e))
  }
...
q) : d == 'meet' ? c.scale(p, p) : d == 'slice' && c.scale(m, m); c.translate(g == null ? 0 : -g, j == null ? 0 : -j)
  ...
  this.attributes = {}; this.styles = {}; this.children = []; this.attribute = function (b, d) {
    var c = this.attributes[b]; if (c != null) {
      return c
    } c = new a.Property(b, ''); d == !0 && (this.attributes[b] = c); return c
  }; this.style = function (b, d) {
    var c = this.styles[b]; if (c != null) {
      return c
    } c = this.attribute(b); if (c != null && c.hasValue()) {
      return c
    } c = this.parent; if (c != null && (c = c.style(b), c != null && c.hasValue())) {
      return c
    }
...
function () {}; this.clearContext = function () {}; this.renderChildren = function (b) {
  ...
  var c = b; d && (c = a.CreateElement(b)); c.parent = this; this.children.push(c)
  ...
  if (b != null) {
    ...
    for (var d = a.compressSpaces(this.attribute('class').value).split(' '), e = 0; e < d.length; e++) {
      b = a.Styles['.' + d[e]]; if (b != null) {
        for (k in b) {
        this.styles[k] = b[k]
      }
      } b = a.Styles[c.nodeName + '.' + d[e]]; if (b != null) {
      for (k in b) {
         this.styles[k] = b[k]
       }
    }
...
c))
  ...
  this.base = a.Element.ElementBase; this.base(c); this.setContext = function (d) {
    if (this.style('fill').Definition.isUrl()) {
      var b = this.style('fill').Definition.getFillStyle(this); if (b != null) {
        d.fillStyle = b
      }
...
this.style('stroke-linecap').value
  ...
  d.font = a.Font.CreateFont(this.style('font-style').value, this.style('font-variant').value, this.style('font-weight').value, this.style('font-size').hasValue() ? this.style('font-size').Length.toPixels() + 'px' : '', this.style('font-family').value).toString()
    ...
    this.attribute('transform').hasValue() && (new a.Transform(this.attribute('transform').value)).apply(d); this.attribute('clip-path').hasValue() && (b = this.attribute('clip-path').Definition.getDefinition(), b != null && b.apply(d)); if (this.style('opacity').hasValue()) {
...
new a.Element.PathElementBase(); a.Element.circle = function (c) {
  this.base = a.Element.PathElementBase; this.base(c); this.path = function (d) {
    var b = this.attribute('cx').Length.toPixels('x'), c = this.attribute('cy').Length.toPixels('y'), e = this.attribute('r').Length.toPixels(); d != null && (d.beginPath(), d.arc(b, c, e, 0, Math.PI * 2, !0), d.closePath()); return new a.BoundingBox(b - e, c - e, b + e, c + e)
  }
...
new a.Element.PathElementBase(); a.Element.line = function (c) {
  this.base = a.Element.PathElementBase; this.base(c); this.getPoints = function () {
    ...
    var b = this.getPoints(); d != null && (d.beginPath(), d.moveTo(b[0].x, b[0].y), d.lineTo(b[1].x, b[1].y)); return new a.BoundingBox(b[0].x, b[0].y, b[1].x, b[1].y)
  }
  this.getMarkers = function () {
    var a = this.getPoints(), b = a[0].angleTo(a[1]); return [[a[0], b], [a[1], b]]
  }
...
this.points[c].y), d != null && d.lineTo(this.points[c].x, this.points[c].y)
    ...
    }a.push([this.points[this.points.length - 1], a[a.length - 1][1]]); return a
  }
...
? !0 : this.tokens[this.i + 1].match(/^[A-Za-z]$/) != null
  ...
  var b = this.getPoint(); return this.current = b
...
this.PathParser; b.reset(); var c = new a.BoundingBox(); for (d != null && d.beginPath(); !b.isEnd();) {
  switch (b.nextCommand(), b.command.toUpperCase()) {
    case 'M':var e = b.getAsCurrentPoint(); b.addMarker(e); c.addPoint(e.x, e.y); d != null && d.moveTo(e.x, e.y); for (b.start = b.current; !b.isCommandOrEnd();) {
      e = b.getAsCurrentPoint(), b.addMarker(e, b.start), c.addPoint(e.x, e.y), d != null && d.lineTo(e.x, e.y)
    } break; case 'L':for (;!b.isCommandOrEnd();) {
      var f = b.current, e = b.getAsCurrentPoint(); b.addMarker(e, f); c.addPoint(e.x, e.y); d != null &&
d.lineTo(e.x, e.y)
    } break; case 'H':for (;!b.isCommandOrEnd();) {
    ...
    } break; case 'V':for (;!b.isCommandOrEnd();) {
    ...
    } break; case 'C':for (;!b.isCommandOrEnd();) {
     var g =
b.current, f = b.getPoint(), j = b.getAsControlPoint(), e = b.getAsCurrentPoint(); b.addMarker(e, j, f); c.addBezierCurve(g.x, g.y, f.x, f.y, j.x, j.y, e.x, e.y); d != null && d.bezierCurveTo(f.x, f.y, j.x, j.y, e.x, e.y)
   } break; case 'S':for (;!b.isCommandOrEnd();) {
...
} break; case 'Q':for (;!b.isCommandOrEnd();) {
...
e = b.getAsCurrentPoint(), b.addMarker(e, j, j), c.addQuadraticCurve(g.x, g.y, j.x, j.y, e.x, e.y), d != null && d.quadraticCurveTo(j.x, j.y, e.x, e.y)
 } break; case 'T':for (;!b.isCommandOrEnd();) {
...
} break; case 'A':for (;!b.isCommandOrEnd();) {
...
}, o = s([1, 0], [(n.x - p.x) / h, (n.y - p.y) / l]), q = [(n.x - p.x) / h, (n.y - p.y) / l], p = [(-n.x - p.x) / h, (-n.y - p.y) / l], n = s(q, p); if (m(q, p) <= -1) {
  n = Math.PI
}m(q, p) >= 1 && (n = 0); j == 0 && n > 0 && (n -= 2 * Math.PI); j == 1 && n < 0 && (n += 2 * Math.PI); q = new a.Point(g.x - h * Math.cos((o + n) /
...
new a.Element.ElementBase(); a.Element.defs = function (c) {
  ...
  this.base = a.Element.ElementBase; this.base(c); this.gradientUnits = this.attribute('gradientUnits').valueOrDefault('objectBoundingBox'); this.stops = []; for (c = 0; c < this.children.length; c++) {
    this.stops.push(this.children[c])
  } this.getGradient = function () {}; this.createGradient = function (d, b) {
    var c = this; this.attribute('xlink:href').hasValue() &&
(c = this.attribute('xlink:href').Definition.getDefinition()); for (var e = this.getGradient(d, b), f = 0; f < c.stops.length; f++) {
...
: this.attribute('fy').Length.toPixels('y')); c = this.gradientUnits == 'objectBoundingBox' ? (c.width() + c.height()) / 2 * this.attribute('r').numValue() : this.attribute('r').Length.toPixels(); return a.createRadialGradient(g, j, 0, e, f, c)
      ...
      this.base = a.Element.ElementBase; this.base(c); a.Animations.push(this); this.duration = 0; this.begin = this.attribute('begin').Time.toMilliseconds(); this.maxDuration = this.begin + this.attribute('dur').Time.toMilliseconds(); this.getProperty = function () {
        var a = this.attribute('attributeType').value, b = this.attribute('attributeName').value; return a == 'CSS' ? this.parent.style(b, !0) : this.parent.attribute(b, !0)
      }; this.initialValue =
null; this.removed = !1; this.calcValue = function () {
  ...
  } this.duration += a; a = !1; if (this.begin < this.duration) {
...
(a = this.attribute('type').value + '(' + a + ')'), this.getProperty().value = a, a = !0
  ...
  return (this.duration - this.begin) / (this.maxDuration - this.begin)
      ...
      this.base = a.Element.AnimateBase; this.base(c); this.calcValue = function () {
        var a = this.attribute('from').numValue(), b = this.attribute('to').numValue(); return a + (b - a) * this.progress()
      }
...
a.Element.animate; this.base(c)
  ...
  this.base = a.Element.ElementBase; this.base(c); this.horizAdvX = this.attribute('horiz-adv-x').numValue(); this.isArabic = this.isRTL = !1; this.missingGlyph = this.fontFace = null; this.glyphs = []; for (c = 0; c < this.children.length; c++) {
    var d = this.children[c]; if (d.type == 'font-face') {
...
new a.Element.ElementBase(); a.Element.missingglyph = function (c) {
  ...
  this.base(c); if (c != null) {
    this.children = []; for (var d = 0; d < c.childNodes.length; d++) {
      var b = c.childNodes[d]; b.nodeType == 1 ? this.addChild(b, !0) : b.nodeType == 3 && this.addChild(new a.Element.tspan(b), !1)
    }
  } this.baseSetContext = this.setContext; this.setContext = function (b) {
    this.baseSetContext(b); if (this.style('dominant-baseline').hasValue()) {
...
1] == ' ') && c < b.length - 2 && b[c + 1] != ' ') {
        g = 'terminal'
      }c > 0 && b[c - 1] != ' ' && c < b.length - 2 && b[c + 1] != ' ' && (g = 'medial'); if (c > 0 && b[c - 1] != ' ' && (c == b.length - 1 || b[c + 1] == ' ')) {
        g = 'initial'
...
l, -1 / l); c.translate(-this.x, -this.y); this.x += k * (h.horizAdvX || b.horizAdvX) / b.fontFace.unitsPerEm; typeof g[j] !== 'undefined' && !isNaN(g[j]) && (this.x += g[j])
    ...
    var b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
      var c = this.parent.style('font-size').numValueOrDefault(a.Font.Parse(a.ctx.font).fontSize),
        k = 0, e = this.getText(); b.isRTL && (e = e.split('').reverse().join('')); for (var f = a.ToNumberArray(this.parent.attribute('dx').value), g = 0; g < e.length; g++) {
          var j = this.getGlyph(b, e, g); k += (j.horizAdvX || b.horizAdvX) * c / b.fontFace.unitsPerEm; typeof f[g] !== 'undefined' && !isNaN(f[g]) && (k += f[g])
        } return k
    }b = a.compressSpaces(this.getText()); if (!c.measureText) {
      return b.length * 10
    } c.save(); this.setContext(c); b = c.measureText(b).width; c.restore(); return b
  }
    ...
    a.Element.tspan = function (c) {
      ...
      this.base = a.Element.TextElementBase; this.base(c); this.getText = function () {
        var a = this.attribute('xlink:href').Definition.getDefinition(); if (a != null) {
          return a.children[0].getText()
        }
      }
    }; a.Element.tref.prototype = new a.Element.TextElementBase()
    a.Element.a = function (c) {
      this.base = a.Element.TextElementBase; this.base(c); this.hasText = !0; for (var d = 0; d < c.childNodes.length; d++) {
        if (c.childNodes[d].nodeType != 3) {
          this.hasText = !1
        }
      } this.text = this.hasText ? c.childNodes[0].nodeValue : ''; this.getText = function () {
        return this.text
...
''); g = g.src.value.split(','); for (j = 0; j < g.length; j++) {
  if (g[j].indexOf('format("svg")') > 0) {
    l = g[j].indexOf('url'); h = g[j].indexOf(')', l); l = g[j].substr(l + 5, h - l - 6); l = a.parseXml(a.ajax(l)).getElementsByTagName('font'); for (h = 0; h < l.length; h++) {
      var o = a.CreateElement(l[h]); a.Definitions[f] = o
    }
  }
}
          }
      ...
      this.base = a.Element.RenderedElementBase; this.base(c); this.baseSetContext = this.setContext; this.setContext = function (a) {
        ...
        var a = this.attribute('xlink:href').Definition.getDefinition(); if (this.attribute('width').hasValue()) {
...
this.getDefinition(); b != null && b.path(a)
      ...
      var b = this.getDefinition(); b != null && b.render(a)
    }
...
function (c) {
  ...
  this.base = a.Element.ElementBase; this.base(c); this.apply = function (a, b) {
    var c = b.getBoundingBox(), e = this.attribute('x').Length.toPixels('x'), f = this.attribute('y').Length.toPixels('y'); if (e == 0 || f == 0) {
      e = c.x1, f = c.y1
    } var g =
this.attribute('width').Length.toPixels('x'), j = this.attribute('height').Length.toPixels('y'); if (g == 0 || j == 0) {
  g = c.width(), j = c.height()
}c = b.style('filter').value; b.style('filter').value = ''; var h = 0.2 * g, l = 0.2 * j, o = document.createElement('canvas'); o.width = g + 2 * h; o.height = j + 2 * l; var n = o.getContext('2d'); n.translate(-e + h, -f + l); b.render(n); for (var q = 0; q < this.children.length; q++) {
  this.children[q].apply(n, 0, 0, g + 2 * h, j + 2 * l)
} a.drawImage(o, 0, 0, g + 2 * h, j + 2 * l, e - h, f - l, g + 2 * h, j + 2 * l); b.style('filter', !0).value = c
  }; this.render =
function () {}
...
s * (m + r * (p == 0 || o == 3 ? 1 : p))
    ...
    var e = this.attribute('stdDeviation').numValue(), c = a.getImageData(0, 0, f, g), e = Math.max(e, 0.01), j = Math.ceil(e * 4) + 1; mask = []; for (var h = 0; h < j; h++) {
      mask[h] = Math.exp(-0.5 * (h / e) * (h / e))
    }e = mask; j = 0; for (h = 1; h < e.length; h++) {
      j += Math.abs(e[h])
    }j = 2 * j + Math.abs(e[0]); for (h = 0; h < e.length; h++) {
      e[h] /= j
    }tmp = []; d(c.data, tmp, e, f, g); d(tmp, c.data, e, g, f); a.clearRect(0, 0, f, g); a.putImageData(c, 0, 0)
  }
...
new a.Element.feGaussianBlur(); a.Element.title = function () {}; a.Element.title.prototype = new a.Element.ElementBase(); a.Element.desc = function () {}; a.Element.desc.prototype = new a.Element.ElementBase(); a.Element.MISSING = function (a) {
  ...
  a.init(c); var b = function (a) {
    for (var b = c.canvas; b;) {
      a.x -= b.offsetLeft, a.y -= b.offsetTop, b = b.offsetParent
    }window.scrollX && (a.x += window.scrollX); window.scrollY && (a.y += window.scrollY); return a
...
k.style('height').Length.toPixels('y'), c.canvas.style.height = c.canvas.height + 'px'
      ...
      } var b = c.canvas.clientWidth || c.canvas.width, d = c.canvas.clientHeight || c.canvas.height; a.ViewPort.SetCurrent(b, d); if (a.opts != null && a.opts.offsetX != null) {
...
a.opts.scaleWidth); k.attribute('height').hasValue() && (g = k.attribute('height').Length.toPixels('y') / a.opts.scaleHeight); k.attribute('width', !0).value = a.opts.scaleWidth; k.attribute('height', !0).value = a.opts.scaleHeight; k.attribute('viewBox', !0).value = '0 0 ' + b * f + ' ' + d * g; k.attribute('preserveAspectRatio', !0).value = 'none'
  ...
  a.intervalID = setInterval(function () {
    var b = !1; g && a.ImagesLoaded() && (g = !1, b = !0); a.opts.ignoreMouse != !0 && (b |= a.Mouse.hasEvents()); if (a.opts.ignoreAnimation != !0) {
      for (var c = 0; c < a.Animations.length; c++) {
        b |= a.Animations[c].update(1E3 / a.FRAMERATE)
      }
    }a.opts != null && typeof a.opts.forceRedraw === 'function' && a.opts.forceRedraw() == !0 && (b = !0); b && (f(), a.Mouse.runEvents())
  }, 1E3 / a.FRAMERATE)
}; a.stop = function () {
...
0
    ...
    for (var b = 0; b < this.events.length; b++) {
      var k = this.events[b]; d.isPointInPath && d.isPointInPath(k.x, k.y) && (this.eventElements[b] = a)
    }
...
this.events[b]; d.isPointInBox(k.x, k.y) && (this.eventElements[b] = a)
    ...
    a.ctx.canvas.style.cursor = ''; for (var c = 0; c < this.events.length; c++) {
...
(function (m) {
  var a = m.css, c = m.CanVGRenderer, d = m.SVGRenderer, b = m.extend, k = m.merge, e = m.addEvent, f = m.createElement, g = m.discardElement; b(c.prototype, d.prototype); b(c.prototype, {create: function (a, b, c, d) {
